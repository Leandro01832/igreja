   ...
   public partial class  Pesquisar
    {
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                modelo = new business.classes.PessoasLgpd.VisitanteLgpd();
                ModificaDataGridView(modelo, tipo, comando);
            }

            if (comboBox1.Text == "Criança")
            {
                modelo = new business.classes.PessoasLgpd.CriancaLgpd();
                ModificaDataGridView(modelo, tipo, comando);
            }

            if (comboBox1.Text == "Membro por aclamação")
            {
                modelo = new business.classes.PessoasLgpd.Membro_AclamacaoLgpd();
                ModificaDataGridView(modelo, tipo, comando);
            }

            if (comboBox1.Text == "Membro por batismo")
            {
                modelo = new business.classes.PessoasLgpd.Membro_BatismoLgpd();
                ModificaDataGridView(modelo, tipo, comando);
            }

            if (comboBox1.Text == "Membro por reconciliação")
            {
                modelo = new business.classes.PessoasLgpd.Membro_ReconciliacaoLgpd();
                ModificaDataGridView(modelo, tipo, comando);
            }

            if (comboBox1.Text == "Membro por trandferência")
            {
                modelo = new business.classes.PessoasLgpd.Membro_TransferenciaLgpd();
                ModificaDataGridView(modelo, tipo, comando);
            }
